...
This library implements ES6 Promises, classes, and subclasses to control various CSS animations. It's currently only setup to work with Angular, but everything is writen in vanilla JavaScript (TypeScript) so it could easily be adapted to work with other frameworks.
...
* `fadeInOverride`: CSS class containing style information needed to execute the fade in. Note: If you change the transition duration, the parameter is required.
...
* In `src/app/slideshow/slideshow.component.ts` use the following code:
